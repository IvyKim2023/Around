...
const (
	INDEX       = "around"
	TYPE        = "post"
	ES_URL      = "http://35.225.68.254:9200"
...
func saveToGCS(ctx context.Context, r io.Reader, bucketName, name string) (*storage.ObjectHandle, *storage.ObjectAttrs, error) {
	...
	// Next check if the buckets exists
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	...
	if days == 0 {
		days = 10 // default to 10 days if not specified
	}
...
func getFriendsHandler(w http.ResponseWriter, r *http.Request) {
	log.Println("Handling request:", r.Method)
...
func deleteFriendHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte("Friend deleted successfully"))
}
...
func deleteFriend(username, friendUsername string) error {
	...
	res, err := client.Get().
		Index(INDEX).
		Type("user").
		Id(username). // Using username as the document ID
		Do()
	if err != nil {
		return err
	}
